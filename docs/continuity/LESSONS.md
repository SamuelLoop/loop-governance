...
## 16. `<ThemeInitScript />` placed directly under `<html>` (sibling of `<body>`) breaks hydration on every page, in both console and admin

**What happened:** Session 7's scaffold wired `console`/`admin`'s root `layout.tsx` as `<html><ThemeInitScript /><body>...</body></html>` — `ThemeInitScript` renders a `next/script` with `strategy="beforeInteractive"`. `<script>` is not a valid direct child of `<html>` in HTML (only `<head>` and `<body>` are) — browsers silently reparent it during initial parse, so the DOM the browser actually builds doesn't match what React's server-rendered output describes at that position. Because this sits in the **root layout**, every single page hydrates against a mismatched tree — not a page-specific bug. Found 2026-08-13 when the real user reported `console.loopcmbntr.live/earnings` "broken" right after the web-10/11 deploys; reproduced on `/earnings`, `/treasury`, and even the untouched `/login` page, which ruled out anything in sessions 09-11's own changes and pointed at something shared by literally every route. Confirmed via production console (`Uncaught ... Minified React error #418`) and, with full detail, by reproducing locally in dev mode (`next dev` prints the real error un-minified: `<%s> cannot contain a nested %s. html <script>`, `In HTML, %s cannot be a child of <%s>. This will cause a hydration error.`) — the minified production error alone doesn't tell you which node mismatches; reproduce locally before guessing.

**Correct pattern:** `next/script` with `strategy="beforeInteractive"` must render inside `<body>`, not as a sibling of it — Next.js still hoists it to execute before hydration regardless of where inside `<body>` it's placed. Fixed in both `apps/console/src/app/layout.tsx` and `apps/admin/src/app/layout.tsx`: moved `<ThemeInitScript />` to be the first child inside `<body>`, after the `className` prop, before `{children}`. `apps/portal` was never affected — portal has no theme toggle and never used `ThemeInitScript` at all.

**Verify root-layout changes with a full dev-server restart, not Fast Refresh:** editing `apps/console/src/app/layout.tsx` and reloading via Next's Fast Refresh did **not** pick up the fix — the exact same hydration error kept reproducing after two separate Fast-Refresh rebuilds. Only killing and restarting `next dev` made the fix take effect. Root layout is a known Fast-Refresh blind spot in Next.js App Router; don't trust a hot-reloaded dev server to confirm a root-layout fix, restart it.
