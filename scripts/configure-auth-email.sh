...
# with a branded magic-link template. Covers console + admin (shared project).
#
...
PAYLOAD=$(RESEND_KEY="$RESEND_KEY" SENDER_EMAIL="$SENDER_EMAIL" SENDER_NAME="$SENDER_NAME" python3 <<'PYEOF'
import json, os
...
echo "Done. Magic link emails now send as $SENDER_NAME <$SENDER_EMAIL>."
